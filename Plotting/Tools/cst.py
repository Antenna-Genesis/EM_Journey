"""
class to deal with data from CST
"""
import pandas as pd
import warnings


class TBPPTable1d(object):
    """
    Result Table from Template based post-Processing
    """
    data_unique_line_allowed = ['CST XY Data Exchange Format V2\n',
                                '----------------------------------------------------------------------\n']
    ...
    def importdata2pd(self):
        ...
        x_list = [[float(item.split()[0])] for item in self.all_lines[2:single_line_num - 1]]
        data_list = x_list
        #  just in case no label name is exported
        try:
            self.plot_xlabel = self.all_lines[0].split()[0]
            self.plot_ylabel = self.all_lines[0].split()[3]
        except:
            print("No xlabel info available from your data!")
            self.plot_xlabel = 'Frequency (GHz)'
            self.plot_ylabel = 'Magnitude (dB)'
