...
######################################################################
#Assign boundary
    def Assignpecbound(self, pecname,name):
...
#                                   		                           "ShowReporterFilter:=", False, "ReporterFilter:=", [False,False], "UseScanAngles:=", True, "Phi:=", Scan_phi, "Theta:=", Scan_theta,
    ...
    def assignLumport(self,faceID,x1,y1,z1,x2,y2,z2):
...
#            reportname=Reportname+' Theta= '+str(Theta)+' deg'
    ...
    def createRectangulzrfarfieldpreportasgaintotal(self, Reportname, Setupname, Theta, Phi, Freq):
            if Theta == 'All':
                theta = 'All'
                phi = str(Phi) + 'deg'
                X_Component = 'Theta'
...
######################################################################### 
    def saveProjectdefault(self):
        ...
        prj_num = 1
        while True:
                path = os.path.join(base_path, 'Prj{}.aedt'.format(prj_num))
