...
0. ## 通用设置
      ...
      0. 路径，文件名，变量名等一律避免出现中文和全角符号

      1. 一般来说，在仿真开始阶段，所有金属结构都设置为PEC，所有介质都无损耗。这一条对于极窄带天线不适用。在加工测试之前，可以把材料换成真实材料，比如copper等，来检查整体的损耗属性。

      2. HFSS是频域求解器，不擅长做宽带计算，进行宽带仿真时使扫频频带尽可能窄，一般不宜超过30%带宽。另外，做相对宽一点频带的仿真时，建议使用离散模式，可以使得远场计算更加准确。

         ![image-20210615121120090](HFSS%E4%BD%BF%E7%94%A8%E6%89%8B%E5%86%8C.assets/image-20210615121120090.png)

      3. 参数化。尽量把所有结构尺寸甚至介电常数都参数化，便于后期做调整、扫描和优化。参数命名方式，参考程序员给代码中的参数命名，建议用下划线法命名，每个变量名中，不超过两个下划线。

      4. 需要生成多个相同结构时候，善用HFSS内置的“duplicate”功能。比如，对于旋转对称的情况（圆周上快速建立均匀分布的几何体），先创建一个几何体，设置好其参数，再点击“duplicate around axis”，选择角度即可复制。*轴对称结构*

      5. 双层空气盒子。尽量设置两层空气盒子，其中，外层盒子与天线所有方向上的距离要约等于1/4中心频率波长，用于吸收电磁波（设置为辐射边界），从而模拟天线放在自由空间的情况；内层尽量接近天线（比如所有方向上离开天线十分之一波长），用作求解方向图时候的近场-远场变换。内边界的表面设置为一个facelist，之后计算远场的时候，选这个facelist作为近场源。

      6. 参数扫描之后绘制plot的过程中，选中optimetrics->parametricssetup1->options->save field and mesh才可以保存之前扫描的结果，把图线画在一起

         ![ ](HFSS%E4%BD%BF%E7%94%A8%E6%89%8B%E5%86%8C.assets/181414_2bf21864_9257194.png)
      ...
      8. 关于同轴线
         ...
         *为什么需要用到同轴线？*
         ...
         ![image-20210624092936681](HFSS使用手册.assets/image-20210624092936681.png)
...
=======
      2. 馈电同轴线的结构图和建模步骤
         ...
         建好同轴结构之后，接下来需要给同轴馈电。有两种馈电方法，第一种是采用lumped port（**请迪凡补充一下激励设置**）

         1. 如上图所示，用GND减去外导体，保证选中GND时外导体对应的圆被减去
=======
      ...
      5. 参数化。尽量把所有结构尺寸甚至介电常数都参数化，便于后期做调整、扫描和优化。参数命名方式，参考程序员给代码中的参数命名，建议用下划线法命名，每个变量名中，不超过两个下划线。
            ...
            ![图片](HFSS%E4%BD%BF%E7%94%A8%E6%89%8B%E5%86%8C.assets/120826788-6164f900-c58d-11eb-82a5-5ed0c9f4f16f.png)
    
         2. 如上图所示，用外导体圆柱减去内导体圆柱，保证选中外导体时中心部分内导体被减去
...
![输入图片说明](https://images.gitee.com/uploads/images/2021/0614/182035_d0eb4a5b_9257194.png "屏幕截图.png")

关于各部分位置与尺寸的细节调整：
初次建模只出现了一个谐振点，查阅资料发现问题：探针高度lh的变化会导致引入电容与电感的大小变化，对输入阻抗影响很大，需要根据实际情况确定lh的长度
因此，贴片的高度，基片厚度，L探针垂直部分长度都需要进行进一步调整才能保证出现两个谐振点
本例各参数为：

GND               100*120*0         中心过原点

patch             30*25*0.07        x,y,z:-15,-1.5,7

lh(探针垂直部分)    r=0.5  h=6.68     x,y,x:0,0,0

 lv(平行部分)        r=0.5  h=8        x,y,z:0,0,6.18

注意输入模型坐标时不要用中文输入法，会报错 ：）
