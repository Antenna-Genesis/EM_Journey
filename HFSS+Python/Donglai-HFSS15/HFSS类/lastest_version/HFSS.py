...
#        self.oDesign = self.oProject.SetActiveDesign()
    ...
    def openProject(self, Path, Projectname):
        ...
        self.oDesktop = self.oAnsoftApp.GetAppDesktop()
        self.oDesktop.RestoreWindow()
        self.oProject = self.oDesktop.OpenProject(Path+"//"+Projectname+".hfss")
        self.oDesign = self.oProject.GetActiveDesign()
        self.oEditor = self.oDesign.SetActiveEditor("3D Modeler")
        self.oModule = self.oDesign.GetModule('BoundarySetup')
        self.transparency = 0.5
...
######################################################################
    ...
    def createcoverlines(self,x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4,name):
...
######################################################################
    ...
    def unitef9(self,n1,n2,n3,n4,n5,n6,n7,n8,n9):
        self.oEditor.Unite(
        	[
        		"NAME:Selections",
        		"Selections:="		, n1+','+n2+','+n3+','+n4+','+n5+','+n6+','+n7+','+n8+','+n9
        	],
        	[
...
#create Mag_E
    def createmage(self,freq,faceid):
        self.oModule = self.oDesign.GetModule("FieldsReporter")
        self.oModule.CreateFieldPlot(
    	[
    		"NAME:Mag_E",
    		...
    		"IntrinsicVar:="	, "Freq=\'"+freq+"\' Phase=\'0deg\'",
    		"PlotGeomInfo:="	, [1,"Surface","FacesList",1,str(faceid)],
    		"FilterBoxes:="		, [0],
    		[
...
######################################################################
    ...
    def createSpreport11(self, Reportname):
...
#            reportname=Reportname+' Theta= '+str(Theta)+' deg'
        ...
        self.oModule.CreateReport(Reportname, "Far Fields", "Rectangular Plot", Setupname+" : LastAdaptive",
	                         ...
	                         item,
                        	 ["X Component:=", X_Component, "Y Component:=", ["dB(RealizedGainPhi)","dB(RealizedGainTheta)"]], [])

    def deleteAllreports(self):
        self.oModule = self.oDesign.GetModule("ReportSetup")
        self.oModule.DeleteAllReports()

    def combine(self,second,first):
        self.oModule = self.oDesign.GetModule("ReportSetup")
        self.oModule.CopyTracesData(second, ["dB(S(1,2))"])
        self.oModule.PasteTraces(first)
    def turnblue(self,plot,item):
        self.oModule = self.oDesign.GetModule("ReportSetup")
        self.oModule.ChangeProperty(
    	[
    		"NAME:AllTabs",
    		[
    			"NAME:Attributes",
    ...
    def exportimage(self,plot,path,name):
        self.oModule = self.oDesign.GetModule("ReportSetup")
        self.oModule.ExportImageToFile(plot, path+"/"+name+".jpg", 0, 0)
######################################################################
...
#        self.oModule.ExportToFile(Reportname, os.path.join(Savepath, Savefilename))
    ...
    def exportTosnp(self, Savepath, Savefilename, Save_num):
        ...
        self.oModule.ExportNetworkData("", ["Setup1:Sweep"], 3, Savepath+"\\"+Savefilename, ["All"], True, 50, "S", -1, 0, 15)
        '''
