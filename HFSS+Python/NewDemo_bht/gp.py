...
class GP(ExactGP):
    def __init__(self, train_x, train_y, likelihood, lengthscale_constraint, outputscale_constraint, ard_dims):
        ...
        base_kernel = MaternKernel(lengthscale_constraint=lengthscale_constraint, ard_num_dims=ard_dims, nu=1.5)
        # 建议用Matern
...
def train_gp(train_x, train_y, use_ard, num_steps, hypers={}):
    ...
    noise_constraint = Interval(1e-5, 5e-5) # 不确定因素，影响比较大，维度低的时候（小于五十），建议忽略。
    if use_ard:
        lengthscale_constraint = Interval(0.001, 1000)
    else:
        lengthscale_constraint = Interval(0.001, math.sqrt(train_x.shape[1]))  # [0.005, sqrt(dim)]
    outputscale_constraint = Interval(0.01, 30)
    ...
    if hypers:
        ...
        hypers["likelihood.noise"] = 0.00002
